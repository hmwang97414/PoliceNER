...
# 将预测的新数据写入文件
def produceData(unlabel_word_lists, unlabel_tag_lists, filename):
    file = open(filename, "a", encoding="utf-8")
    print(3)
    for i in range(len(unlabel_word_lists)):
        for j in range(len(unlabel_word_lists[i])):
            # print(unlabel_word_lists[i][j] + " " + unlabel_tag_lists[i][j])
            file.write(unlabel_word_lists[i][j] + " " + unlabel_tag_lists[i][j] + "\n")
        file.write("\n")
    file.close()
